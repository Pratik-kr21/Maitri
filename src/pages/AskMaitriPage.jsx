...
export default function AskMaitriPage() {
    ...
    const loadHistory = useCallback(async () => {
        ...
        try {
            const res = await api.get('/ask-maitri/history?days=14');
    ...
    const openHistoryDay = (record) => {
        setSelectedDate(record.date);
        setMessages(record.messages.map(m => ({ ...m, ts: new Date(m.ts).getTime() })));
    ...
    const sendMessage = async (text) => {
        ...
        if (selectedDate) { setSelectedDate(null); } // switch to live session
    ...
    return (
        <div className="flex h-full overflow-hidden bg-[#F8F4F5]">
            ...
            <aside className={`flex-shrink-0 bg-white border-r border-[#EDE0E3] flex flex-col transition-all duration-300 overflow-hidden
                ...
                <div className="flex items-center justify-between px-4 py-3 border-b border-[#EDE0E3] flex-shrink-0">
                    <h2 className="text-sm font-semibold text-[#2C1A1D]">Chat History</h2>
                    <button onClick={() => setShowHistory(false)}
                        className="text-[#9E7A82] hover:text-[#2C1A1D] text-lg leading-none">×</button>
                </div>

                <button onClick={newChat}
                    className="mx-3 my-2 py-2 px-3 rounded-lg border border-[#EDE0E3] text-xs font-semibold text-[#E87A86]
                        hover:bg-[#FDE0E4] transition-colors flex items-center gap-2 flex-shrink-0">
                ...
                <div className="flex-1 overflow-y-auto px-2 pb-3">
                    {loadingHistory ? (
                        <div className="flex flex-col gap-2 px-2 pt-2">
                            {[1, 2, 3].map(i => <div key={i} className="h-12 rounded-lg bg-[#F8F4F5] animate-pulse" />)}
                        </div>
                    ) : chatHistory.length === 0 ? (
                        <p className="text-xs text-[#9E7A82] text-center py-6 px-3 leading-relaxed">
                            Your past conversations will appear here.
                        </p>
                    ) : (
                        chatHistory.map(record => {
                            const firstQ = record.messages.find(m => m.role === 'user')?.content || '';
                            const isActive = selectedDate === record.date;
                            return (
                                <button key={record.date} onClick={() => openHistoryDay(record)}
                                    className={`w-full text-left px-3 py-2.5 rounded-lg mb-1 transition-colors
                                        ${isActive
                                            ? 'bg-[#FDE0E4] border border-[#E87A86]/30'
                                            : 'hover:bg-[#F8F4F5] border border-transparent'}`}>
                                    <p className="text-[10px] font-semibold text-[#9E7A82] mb-0.5">
                                        {formatDate(record.date)} · {record.messages.filter(m => m.role === 'user').length} questions
                                    </p>
                                    <p className="text-xs text-[#2C1A1D] line-clamp-2 leading-relaxed">{firstQ}</p>
                                </button>
                            );
                        })
                    )}
            ...
            <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
                ...
                {/* Header */}
                <div className="bg-white border-b border-[#EDE0E3] px-4 py-3 flex-shrink-0">
                    <div className="w-full flex items-center justify-between gap-4">
                        <div className="flex items-center gap-3">
                            {/* History toggle */}
                            <button onClick={() => { setShowHistory(s => !s); if (!showHistory) loadHistory(); }}
                                className={`p-2 rounded-lg transition-colors flex-shrink-0
                                    ${showHistory ? 'bg-[#FDE0E4] text-[#E87A86]' : 'text-[#9E7A82] hover:bg-[#F8F4F5] hover:text-[#2C1A1D]'}`}
                                title="Chat history">
                            ...
                            <div>
                                <div className="flex items-center gap-2">
                                    <h1 className="font-heading text-lg font-semibold text-[#2C1A1D]">Ask Maitri</h1>
                                    {selectedDate && (
                                        <span className="px-2 py-0.5 bg-[#FDE0E4] text-[#C85C6B] text-[10px] font-semibold rounded-full">
                                            {formatDate(selectedDate)} — read-only
                                        </span>
                                    )}
                                </div>
                        ...
                        <div className="flex items-center gap-3 flex-shrink-0">
                            {selectedDate ? (
                                <button onClick={newChat}
                                    className="px-3 py-1.5 text-xs font-semibold text-[#E87A86] border border-[#E87A86]/30 rounded-lg hover:bg-[#FDE0E4] transition-colors">
                                    New chat
                                </button>
                            ) : (
                                <>
                ...
                {selectedDate && (
                    <div className="border-t border-[#EDE0E3] bg-[#F8F4F5] px-6 py-3 flex-shrink-0 flex items-center justify-between">
                        <p className="text-xs text-[#9E7A82]">You're viewing a past conversation from <strong>{formatDate(selectedDate)}</strong>.</p>
                        <button onClick={newChat}
                            className="text-xs font-semibold text-[#E87A86] hover:text-[#D66874] transition-colors">
                            Start new chat →
                        </button>
                    </div>
                )}
